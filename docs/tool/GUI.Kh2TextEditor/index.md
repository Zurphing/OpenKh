...
| 15   | {:chardelay Value} | Used on text that precedes selectable options that teleport the player to fights, like Absent Silhouettes. The value is the Program ID, and it specifies which forced fight to warp to. Otherwise, a value of 0x33 will serve as a "cancel" option to exit out of the textbox.
